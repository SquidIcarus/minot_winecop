...
{ /* ANCHOR - Music */ }
      ...
      {music && music.length > 0 && (
        <section style={{ padding: '4rem 2rem', borderBottom: '1px solid #222' }}>
          ...
          <p style={{ fontSize: 'clamp(1.5rem, 4vw, 2.5rem)', fontWeight: 700 }}>{music[0].title}</p>
          <p style={{ opacity: 0.5, marginTop: '0.5rem' }}>{music[0].type} — {music[0].release_date}</p>
          <div style={{ display: 'flex', gap: '1rem', marginTop: '1.5rem', flexWrap: 'wrap' }}>
            {music[0].bandcamp_url && (
              <a href={music[0].bandcamp_url} target="_blank" rel="noopener noreferrer"
                style={{ color: '#f0ece4', borderBottom: '1px solid #f0ece4', paddingBottom: '2px', textDecoration: 'none', fontSize: '0.9rem', letterSpacing: '0.1em' }}>
                Bandcamp ↗
              </a>
            )}
            {music[0].spotify_url && (
      ...
      <section style={{ padding: '4rem 2rem', borderBottom: '1px solid #222' }}>
        ...
        {gigs && gigs.length > 0 ? (
          <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            {gigs.map((gig) => (
              <li key={gig.id} style={{ display: 'flex', gap: '2rem', alignItems: 'baseline', flexWrap: 'wrap', borderBottom: '1px solid #1a1a1a', paddingBottom: '1.5rem' }}>
                <span style={{ fontFamily: 'monospace', opacity: 0.5, minWidth: '120px' }}>{gig.date}</span>
                <span style={{ fontWeight: 700 }}>{gig.venue}</span>
                <span style={{ opacity: 0.5 }}>{gig.city}{gig.state ? `, ${gig.state}` : ''}</span>
                {gig.ticket_url && (
                  <a href={gig.ticket_url} target="_blank" rel="noopener noreferrer"
                    style={{ marginLeft: 'auto', color: '#f0ece4', border: '1px solid #333', padding: '0.25rem 0.75rem', textDecoration: 'none', fontSize: '0.8rem', letterSpacing: '0.1em' }}>
                    Tickets ↗
                  </a>
                )}
              </li>
